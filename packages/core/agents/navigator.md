...
- Treat each `<session_command agent="AGENT_NAME">...</session_command>` block as literal input.
- Render variables, then call `session_command` with `input` set to the rendered body and `agent` set to the tag value.
