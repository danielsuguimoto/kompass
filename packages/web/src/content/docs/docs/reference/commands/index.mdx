...
- Core: `/ask`, `/branch`, `/commit`, `/commit-and-push`, `/learn`, `/merge`, `/rmslop`
...
- navigator workflows orchestrate locally and queue focused leaf work with literal `<session_command>` blocks
...
- expected tools: `ticket_load`, `session_command(dev)`, `session_command(branch)`, `session_command(commit-and-push)`, `session_command(pr/create)`
...
- expected tools: `session_command(branch)`, `session_command(commit)`, `session_command(pr/create)`
...
- expected tools: `read`, `question`, `session_command(ticket/plan)`, `session_command(dev)`, `session_command(commit)`
